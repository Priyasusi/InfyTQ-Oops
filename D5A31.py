...
class Logistics(metaclass=ABCMeta):
    ...
    def generate_consumer_id(self):
        ...
        Logistics.__counter=int(Logistics.__counter)+1
...
class PassengerLogistics(Logistics):
    ...
    def validate_vehicle_type(self):
        ...
        return -1
    def calculate_bill_amount(self):
        if(self.validate_vehicle_type()>-1):
            vehi=True
        else:
            vehi=False
        if(vehi and self.validate_meter_reading()):
            super().generate_consumer_id()
            distance_travelled=self.get_end_reading()-self.get_start_reading()
            ind=self.validate_vehicle_type()
            if(distance_travelled<=100):
                charge=PassengerLogistics.__list_charge_for_hundred[ind]*distance_travelled
            else:
                charge=PassengerLogistics.__list_charge_for_hundred[ind]*100+PassengerLogistics.__list_charge_after_hundred[ind]*(distance_travelled-100)
            if(charge<PassengerLogistics.__list_minimum_charge[ind]):
                charge=PassengerLogistics.__list_minimum_charge[ind]
    ...
    def calculate_bill_amount(self):
        if(self.validate_carrier_type()):
            if(self.validate_meter_reading()):
                ...
                bill_amount=bill_amount+(bill_amount*(10/100))+2000
                return bill_amount
            else:
                return -1
        else:
...
goods_logistic=GoodsLogistics("EICHER",3000,300,400)
