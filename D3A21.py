'''Care hospital management wants to calculate the charge of lab tests done by its patients.30 min
Write a python program to implement the class diagram given below.
Class Description:
LabTestRepository class:
list_of_hospital_lab_test_ids: Static list which contains the list of test ids of lab tests available
in the hospital
...
get_test_charge(lab_test_id): Accept a lab test id and return the corresponding lab test charge.
If lab test id is invalid, return -1
Patient class:
list_of_lab_test_ids: Instance variable which contains the list of test ids of lab tests done
by the patient
...
class Patient:
    ...
    def calculate_lab_test_charge(self):
	    ...
	    total_test_charge=0
	    for t_id in self.__list_of_lab_test_ids:
	        v1=LabTestRepository.get_test_charge(t_id)
	        if(v1!=-1):
	            total_test_charge+=v1
	    self.__lab_test_charge=total_test_charge
